...
// `jest.replaceProperty(Platform, 'OS', 'web')` below outlives the test that sets it otherwise.
afterEach(() => {
...
test('tab bars render appropriate hrefs on web', () => {
  ...
  renderRouter({
    _layout: () => (
      <Tabs screenOptions={{ tabBarButton: ({ href }) => <Text>{href}</Text> }}>
        <Tabs.Screen name="index" />
  ...
  expect(screen.getByText('/nested/second')).not.toBeNull();
});
...
// A custom `tabBar` is a public API. It receives the tab state, the descriptors, and the discrete
// `emit` and `navigateToTab` props that replaced the navigation object.
test('renders a custom tabBar and lets it emit and navigate', async () => {
  function CustomTabBar({ state, descriptors, emit, navigateToTab }: BottomTabBarProps) {
    return (
...
test('lets a tabPress listener prevent navigation from a custom tabBar', async () => {
  ...
  function CustomTabBar({ state, emit, navigateToTab }: BottomTabBarProps) {
    return (
      <View>
        {state.routes.map((route) => (
          <Text
