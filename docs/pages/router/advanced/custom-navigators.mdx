...
- **`processScreens`**: transforms the screens declared as children of your navigator before they are rendered. Use it to support shortcut props on `<Navigator.Screen>` by rewriting them into real screen options.
...
`processScreens` receives the screens declared as children of your navigator and returns the screens to render. Their options are typed with your navigator's options, so declare the shortcut prop there. The example below turns a `badge` shortcut prop into a `title` suffix:
...
`processScreens` runs on every render, after `Protected` children are flattened, and receives the declared screens only. Routes Expo Router inferred from the filesystem are not passed to it.
